...
- 标准插孔：**8.60 × 4.60**；孔深留约 0.7 mm 余量，不以孔底作为主要定位面。
...
门头位置较旧版上移，整套模型最高 Z = **104**。新版屋顶厚 8 mm，用来容纳插孔；后续可根据刚度与耗材消耗进一步减薄/镂空。
...
- 0.30 是滑动间隙初始值，**不是过盈卡紧量**。目前没有弹性锁扣/磁吸，不能承诺搬动时不会滑出；移动整套模型前应取下品牌件，勿倒置。
- 先试打小样，之后决定是否加入小摩擦筋或其他防松结构；不靠盲目缩小整个导槽解决防松。

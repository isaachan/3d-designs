...
const decks=[66,129,192].map(h=>`<rect x="${x(0)}" y="${z(h+5)}" width="${360*S}" height="${5*S}" fill="#657a8a"/>`).join('');
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1250" height="390" viewBox="0 0 1250 390"><rect width="100%" height="100%" fill="white"/><text x="45" y="32" font-family="sans-serif" font-size="22" font-weight="bold">v2 装配正视预览：五个一体模块与四联燕尾榫</text>${modules}${seams}<rect x="${x(0)}" y="${z(8)}" width="${920*S}" height="${8*S}" fill="#4e5965"/>${decks}<line x1="${x(10)}" y1="${z(8)}" x2="${x(10)}" y2="${z(230)}" stroke="#d6a115" stroke-width="5"/><line x1="${x(340)}" y1="${z(8)}" x2="${x(340)}" y2="${z(230)}" stroke="#d6a115" stroke-width="5"/><rect x="${x(360)}" y="${z(238)}" width="${6*S}" height="${230*S}" fill="#45525e"/><rect x="${x(914)}" y="${z(238)}" width="${6*S}" height="${230*S}" fill="#45525e"/><text x="45" y="365" font-family="sans-serif" font-size="14">灰：主体模块；黄：车库承托柱；橙：每界面两底板＋两背板滑入燕尾榫</text></svg>`;

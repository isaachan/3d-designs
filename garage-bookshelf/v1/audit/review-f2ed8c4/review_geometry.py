"""Independent, read-only review of f2ed8c4; writes only this review directory.
...
def solid(t):
    converted=[]
    for m in Mesh.Mesh(t.tolist()).getSeparateComponents():
...
solids={n:solid(t) for n,t in world.items() if not n.startswith(('12_','13_'))}
clashes=[]
...
ax=axs[0]
ax.add_patch(Rectangle((905,123),3,70,color='#edaa32'))
ax.add_patch(Rectangle((914,8),6,150,color='#8694a2'))
ax.add_patch(Rectangle((914,158),6,150,color='#a0afbd'))
ax.annotate('',xy=(908,165),xytext=(914,165),arrowprops=dict(arrowstyle='<->',color='#ca2828'))
ax.text(908.7,170,'6 mm',color='#ca2828');ax.axhline(158,color='#778',ls=':',lw=.6)
ax.set(xlim=(899,925),ylim=(115,203),xlabel='X / mm',ylabel='Z / mm',title='Right side splice: no contact');ax.set_aspect('equal')
ax=axs[1]
ax.add_patch(Rectangle((214,420),6,70,color='#29313b'))
ax.add_patch(Rectangle((223,436),3,38,color='#edaa32'))
ax.annotate('',xy=(220,450),xytext=(223,450),arrowprops=dict(arrowstyle='<->',color='#ca2828'))
ax.text(220.3,457,'3 mm',color='#ca2828')
ax.set(xlim=(210,230),ylim=(415,495),xlabel='Y / mm',ylabel='Z / mm',title='Billboard splice: no contact');ax.set_aspect('equal')
fig.tight_layout();fig.savefig(OUT/'connection-gaps.png',dpi=160);plt.close(fig)
...
out=dict(stl_count=len(list((SRC/'stl').glob('*.stl'))),mesh_count=len(records),meshes=records,clashes=clashes,contacts=contacts,projects=projects)
